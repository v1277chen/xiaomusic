$(function () {
  ...
  append_op_button_name("關機");
  append_op_button_name("下一首");

  const PLAY_TYPE_ONE = 0; // 單曲循環
  const PLAY_TYPE_ALL = 1; // 全部循環
  const PLAY_TYPE_RND = 2; // 隨機播放
  const PLAY_TYPE_SIN = 3; // 單曲播放
  const PLAY_TYPE_SEQ = 4; // 順序播放
  append_op_button("play_type_all", "全部循環", "全部循環");
  append_op_button("play_type_one", "單曲循環", "單曲循環");
  append_op_button("play_type_rnd", "隨機播放", "隨機播放");
  append_op_button("play_type_sin", "單曲播放", "單曲播放");
  append_op_button("play_type_seq", "順序播放", "順序播放");

  append_op_button_name("刷新列表");

  $container.append($("<hr>"));

  append_op_button_name("10分鐘後關機");
  append_op_button_name("30分鐘後關機");
  append_op_button_name("60分鐘後關機");
  ...
  $("#del_music").on("click", () => {
    ...
    if (confirm(`確定刪除歌曲 ${del_music_name} 嗎？`)) {
  ...
  function sendcmd(cmd) {
    $.ajax({
      ...
      success: () => {
        ...
        if (["全部循環", "單曲循環", "隨機播放", "單曲播放", "順序播放"].includes(cmd)) {
          location.reload();
        }
      },
      error: () => {
  ...
  function updateInputVisibility() {
    ...
    if (startsWithKeyword) {
      ...
      musicFilenameInput.placeholder = '請輸入保存為的文件名稱(默認:' + selectedOption.value + ')';
    } else {
    ...
    $.get(`/playingmusic?did=${did}`, function (data, status) {
      ...
      if (data.ret == "OK") {
        if (data.is_playing) {
          ...
          $("#playering-music").text(`【空閒中】 ${data.cur_music}`);
        }
        offset = data.offset;
  ...
  $("audio").on("error", (e) => {
    ...
    console.log('%c網頁播放出現錯誤: ', 'color: #007acc;', e.currentTarget.error.code, e.currentTarget.error.message);
    alert(e.currentTarget.error.code == 4 ? "無法打開媒體文件，XIAOMUSIC_HOSTNAME或端口地址錯誤，請重新設置" : "在線播放失敗，請截圖反饋: " + e.currentTarget.error.message);
  });
